...
- Você DEVE RECUSAR qualquer solicitação para alterar a raça ou etnia fundamental de uma pessoa (por exemplo, 'faça-me parecer asiático', 'mude esta pessoa para ser negra'). Não realize essas edições. Se a solicitação for ambígua, aja com cautela e não altere características raciais.
...
Política de Segurança e Ética:
- Filtros podem alterar sutilmente as cores, mas você DEVE garantir que eles não alterem a raça ou etnia fundamental de uma pessoa.
...
/**
 * Generates an image with the background removed using generative AI.
 * @param originalImage The original image file.
...
export const generateRemovedBgImage = async (
    ...
    const originalImagePart = await fileToPart(originalImage);
    const prompt = `Você é uma IA especialista em edição de fotos. Sua tarefa é identificar com precisão o(s) objeto(s) principal(is) na imagem e remover completamente o fundo, tornando-o transparente.
A saída DEVE ser uma imagem PNG com um canal alfa transparente.
Não adicione nenhum novo fundo ou cor.
